    ...
    public class Group 
    {
        public class GroupNameComparer : IComparer<Group> // пример необобщённого интерфейса
        {
            public int Compare(object ob1, object ob2)
            {
                Group m1 = (Group)ob1;
                Group m2 = (Group)ob2;
                if (m1.NameGroup.Length > m2.NameGroup.Length) return 1;
                if (m1.NameGroup.Length < m2.NameGroup.Length) return -1;
                if (m1.NameGroup.Length == m2.NameGroup.Length) return 0;
            }
        ...
        public class NumberGroupeComparer : IComparer<Group> // пример необобщённого интерфейса
        {
            public int Compare(object ob1, object ob2)
            {
                Group m1 = (Group)ob1;
                Group m2 = (Group)ob2;
                if (m1.NumberCourse > m2.NumberCourse) return 1;
                if (m1.NumberCourse < m2.NumberCourse) return -1;
                if (m1.NumberCourse == m2.NumberCourse) return 0;
            }
            //public int Compare(Student x, Student y)
            //{
            //    if (x.CompareTo(y) != 0)
